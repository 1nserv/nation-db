...
def error(ip: str, method: str, path: str, code: int = 500, message: str = "Unknown Error"):
	...
	output = f"\033[1;34m{ip}\033[0m" + (16 - len(ip)) * " " + f"at {date} | \033[31;1m{code}\033[0m {method} {path}"
	print(output)

	with open(os.path.join(logpath, "requests.log"), "a") as file:
		file.write(f"\n{ip}" + (16 - len(ip)) * " " + f"at {date} | {method} {path} - {code} {message}")

def log(ip: str, method: str, path: str, code: int = 200, message: str = "Success"):
	date = datetime.now().strftime("%d/%m/%Y %H:%M:%S")

	output = f"\033[1;34m{ip}\033[0m" + (16 - len(ip)) * " " + f"at {date} | \033[32;1m{code}\033[0m {method} {path}"
	print(output)
